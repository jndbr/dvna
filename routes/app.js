...
// Rate limit for admin usersapi (limit each IP to 10 requests per minute)
...
// Rate limiter for useredit endpoint (limit each IP to 5 requests per minute)
var userEditLimiter = rateLimit({
  windowMs: 60 * 1000, // 1 minute
  max: 5, // limit each IP to 5 requests per windowMs
  message: "Too many user edit requests, please try again later."
...
module.exports = function () {
    ...
    router.get('/admin', adminLimiter, authHandler.isAuthenticated, function (req, res) {
        res.render('app/admin', {
            admin: (req.user.role == 'admin')
        })
    })

    router.get('/admin/usersapi', adminUsersApiLimiter, authHandler.isAuthenticated, appHandler.listUsersAPI)
    ...
    router.post('/usersearch', userSearchLimiter, authHandler.isAuthenticated, appHandler.userSearch)
    ...
    router.post('/useredit', userEditLimiter, authHandler.isAuthenticated, appHandler.userEditSubmit)
